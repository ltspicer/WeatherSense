...
def main():
    ...
    if MQTT_ACTIVE:
        ...
        for s in sensor_data:
            ...
            key = f"Type{type_}-Channel{channel}{prefix}"
            base = f"devData/{key}"

            if cur_val is not None and cur_val != 65535 and cur_val != 255:
                send_mqtt(client, f"{base}/current", c_f_berechnen(cur_val, prefix))
            if high_val is not None and high_val != 65535 and high_val != 255:
                send_mqtt(client, f"{base}/high", c_f_berechnen(high_val, prefix))
            if low_val is not None and low_val != 65535 and low_val != 255:
                send_mqtt(client, f"{base}/low", c_f_berechnen(low_val, prefix))

            for k, v in s.items():
                if k.startswith("dev") and v not in (None, {}):
                    send_mqtt(client, f"{base}/{k}", v)
